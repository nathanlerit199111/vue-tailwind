  ...
  <div class="layout-creator">
    ...
    <div class="step" v-if="columns.length > 0">
      ...
      <div v-for="(column, index) in columns" :key="index" class="column-config">
        ...
        <!-- <label :for="'nested-elements-' + (index + 1)">Enter number of nested elements in Column {{ index + 1 }}:</label>
        <input
          type="number"
          v-model.number="column.numNestedElements"
          :id="'nested-elements-' + (index + 1)"
          min="0"
          @input="handleNestedElementsChange(index)"
        /> -->

        <div v-if="column.nestedElements.length > 0">
          <h4>Configure Nested Elements in Column {{ index + 1 }}:</h4>
          <div v-for="(nestedElement, nestedIndex) in column.nestedElements" :key="nestedIndex" class="nested-element-config">
            <label :for="'nested-element-' + (index + 1) + '-' + (nestedIndex + 1)">
              Nested Element {{ nestedIndex + 1 }}
            </label>
            <input
              type="text"
              v-model="nestedElement.name"
              :id="'nested-element-' + (index + 1) + '-' + (nestedIndex + 1)"
            ...
            <div class="alignment-options">
              <label>
                <input
                  type="radio"
                  :name="'nested-align-' + (index + 1) + '-' + (nestedIndex + 1)"
                  value="center"
                  v-model="nestedElement.alignment"
                />
                Center
              </label>
              <label>
    ...
    <div class="step" v-if="columns.length > 0">
      ...
      <div class="generated-layout">
        <div class="parent">
          ...
          >
            <p>{{ column.name }}</p>
            <!-- Nested elements within each column -->
            <div
              v-for="(nestedElement, nestedIndex) in column.nestedElements"
              :key="nestedIndex"
              :class="'nested-child nested-child-' + (index + 1) + '-' + (nestedIndex + 1)"
              :style="{ justifyContent: nestedElement.alignment }"
            >
              {{ nestedElement.name }}
            </div>
          </div>
...
// Function to handle change in number of nested elements for a specific column
const handleNestedElementsChange = (columnIndex) => {
  const column = columns.value[columnIndex];
  column.nestedElements = Array.from({ length: column.numNestedElements }, (_, i) => ({
...
<div class="parent">
  ${columns.value
    ...
    <div class="child child-${index + 1}">
      <p>${column.name}</p>
      ${column.nestedElements
        .map(
          (nestedElement, nestedIndex) => `
      <div class="nested-child nested-child-${index + 1}-${nestedIndex + 1}">
        ${nestedElement.name}
      </div>`
        )
        .join('')}
...
${column.nestedElements
  .map(
    (nestedElement, nestedIndex) => `
.nested-child-${index + 1}-${nestedIndex + 1} {
  display: flex;
  justify-content: ${nestedElement.alignment};
